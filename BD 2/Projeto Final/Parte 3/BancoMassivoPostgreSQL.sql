/* ==========================================================
DML CORRIGIDO E PREPARADO - POSTGRESQL
==========================================================
*/
SET search_path TO escola_idiomas;

-- 1. Limpeza Total (Usando CASCADE para limpar dependentes)
TRUNCATE TABLE avaliacoes, mensalidades, matriculas, turmas, cursos, professores, alunos, pessoas, salas, idiomas, formas_pagamento RESTART IDENTITY CASCADE;
...
(6, 'Auditório Massivo A', 5000, 'Auditório'), -- ID 6 (Para Carga)
(7, 'Auditório Massivo B', 5000, 'Auditório'); -- ID 7 (Para Carga)
...
-- 5. Pessoas (Professores - IDs 1 a 5)
...
-- 8. MATRÍCULAS COM IDS FORÇADOS (1 a 39)
...
(28, 33, 1, '2024-03-01', 1800.00, 'Ativa'), 
...
-- 9. AQUI VOCÊ DEVE COLAR O SEU INSERT GIGANTE DE MENSALIDADES (MANUAIS) QUE VOCÊ TEM.
--    O PostgreSQL vai aceitar normalmente, pois os IDs 1 a 39 agora existem.
...
/* =========================================================================
   PASSO CRÍTICO: AJUSTE DAS SEQUENCES (CONTADORES)
   =========================================================================
   Como inserimos IDs manualmente, precisamos dizer ao Postgres para
   começar a contar a partir do maior número que inserimos.
   Caso contrário, a procedure de carga tentará usar ID 1, 2, 3... e dará erro.
*/
-- Ajusta contadores (assumindo nomes padrão do SERIAL. Se der erro, verifique o nome da sequence)
SELECT setval('escola_idiomas.pessoas_pessoa_id_seq', (SELECT MAX(pessoa_id) FROM pessoas));
SELECT setval('escola_idiomas.matriculas_matricula_id_seq', (SELECT MAX(matricula_id) FROM matriculas));
SELECT setval('escola_idiomas.avaliacoes_avaliacao_id_seq', (SELECT MAX(avaliacao_id) FROM avaliacoes));
...
*/
/* ==========================================================
PROCEDURE DE CARGA MASSIVA - POSTGRESQL
==========================================================
*/
SET search_path TO escola_idiomas;

CREATE OR REPLACE PROCEDURE sp_carga_massiva(p_quantidade INT)
LANGUAGE plpgsql
AS $$
DECLARE
    v_i INT;
    v_novo_aluno_id INT;
    v_nova_matricula_id INT;
    v_turma_massiva_id INT;
BEGIN
    -- Loop para inserir a quantidade desejada
    FOR v_i IN 1..p_quantidade LOOP
        
        -- 1. Inserir Pessoa e capturar o ID gerado (RETURNING)
        INSERT INTO pessoas (nome, data_nasc, cpf, email, telefone, endereco) 
        VALUES (
            'Aluno Massivo ' || v_i, 
            '2000-01-01', 
            'CPF' || lpad(v_i::text, 8, '0'), 
            'aluno' || v_i || '@massivo.com', 
            '31999999999', 
            'Rua Massiva'
        )
        RETURNING pessoa_id INTO v_novo_aluno_id;
        
        -- Inserir na tabela Alunos
        INSERT INTO alunos (aluno_id, nivel_proeficiencia_inicial) 
        VALUES (v_novo_aluno_id, 'Básico');
        
        -- 2. Escolher uma turma massiva aleatoriamente (ID 20 ou 21)
        IF (random() > 0.5) THEN
            v_turma_massiva_id := 20; -- Turma Ingles Massiva
        ELSE
            v_turma_massiva_id := 21; -- Turma Espanhol Massiva
        END IF;

        -- 3. Inserir Matrícula e capturar ID
        INSERT INTO matriculas (aluno_id, turma_id, data_matricula, valor_total_curso, status_mat)
        VALUES (v_novo_aluno_id, v_turma_massiva_id, CURRENT_DATE, 2000.00, 'Ativa')
        RETURNING matricula_id INTO v_nova_matricula_id;

        -- 4. USAR SUA REGRA DE NEGÓCIO para gerar o financeiro
        -- Chama a procedure existente passando o ID da nova matrícula
        CALL escola_idiomas.sp_gerar_mensalidades(v_nova_matricula_id);
        
        -- 5. Gerar Avaliação Aleatória (0 a 10)
        INSERT INTO avaliacoes (matricula_id, professor_id, data_avaliacao, tipo_avaliacao, nota)
        VALUES (
            v_nova_matricula_id, 
            1, 
            CURRENT_DATE, 
    ...
    RAISE NOTICE 'Carga concluída! % alunos inseridos com sucesso.', p_quantidade;
END;
$$;
