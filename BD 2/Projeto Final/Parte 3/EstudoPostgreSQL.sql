...
SELECT pg_size_pretty(pg_total_relation_size('escola_idiomas.alunos')) as "Espaço Ocupado por Alunos";
SELECT pg_size_pretty(pg_total_relation_size('escola_idiomas.avaliacoes')) as "Espaço Ocupado por Avaliações";
SELECT pg_size_pretty(pg_total_relation_size('escola_idiomas.cursos')) as "Espaço Ocupado por Cursos";
SELECT pg_size_pretty(pg_total_relation_size('escola_idiomas.formas_pagamento')) as "Espaço Ocupado por Formas de Pagamento";
SELECT pg_size_pretty(pg_total_relation_size('escola_idiomas.idiomas')) as "Espaço Ocupado por Idiomas";
SELECT pg_size_pretty(pg_total_relation_size('escola_idiomas.matriculas')) as "Espaço Ocupado por Matrículas";
SELECT pg_size_pretty(pg_total_relation_size('escola_idiomas.mensalidades')) as "Espaço Ocupado por Mensalidades";
SELECT pg_size_pretty(pg_total_relation_size('escola_idiomas.pessoas')) as "Espaço Ocupado por Pessoas";
SELECT pg_size_pretty(pg_total_relation_size('escola_idiomas.professores')) as "Espaço Ocupado por Professores";
SELECT pg_size_pretty(pg_total_relation_size('escola_idiomas.salas')) as "Espaço Ocupado por Salas";
SELECT pg_size_pretty(pg_total_relation_size('escola_idiomas.turmas')) as "Espaço de Turmas";
-- =================================================================================================================================================
...
-- ANÁLISE DA CONSULTA
EXPLAIN (ANALYZE, BUFFERS) select pe.nome as "Nome do professor", pr.especialidade as "Especialidade", 
round(avg(av.nota), 2) as "Média geral das notas", count(av.avaliacao_id) as "Número total de avaliações"
from pessoas pe join professores pr on pr.professor_id = pe.pessoa_id
join avaliacoes av on av.professor_id = pr.professor_id group by pe.nome, pr.especialidade
order by avg(av.nota) desc;
...
EXPLAIN (ANALYZE, BUFFERS) select cu.nome_curso as "Nome do curso", sum(me.valor_nominal) as "Valor nominal de parcelas vencidas", 
sum(case when me.status_pag in ('Pendente', 'Vencido') then me.valor_nominal else 0 end) as "Inadimplência Real"
from mensalidades me join matriculas ma on ma.matricula_id = me.matricula_id 
join turmas tu on tu.turma_id = ma.turma_id join cursos cu on cu.curso_id = tu.curso_id 
where (me.data_vencimento <= now()) group by cu.nome_curso order by cu.nome_curso;
...
-- ANÁLISE DA VARIAÇÃO 2
EXPLAIN (ANALYZE, BUFFERS) select sum(me.valor_nominal) as "Valor nominal de parcelas vencidas" 
...
EXPLAIN (ANALYZE, BUFFERS) select t.nome_turma, s.nome_sala, s.capacidade, COUNT(m.matricula_id) as "Alunos Matriculados"
from turmas t join salas s on t.sala_id = s.sala_id join matriculas m on t.turma_id = m.turma_id
where m.status_mat = 'Ativa' group by t.nome_turma, s.nome_sala, s.capacidade
having COUNT(m.matricula_id) > (s.capacidade * 0.8);
-- -------------------------------------------------------------------------------------------------------------------------------------------------
...
-- ANÁLISE DA VARIAÇÃO 1
EXPLAIN (ANALYZE, BUFFERS) select t.nome_turma, COUNT(m.matricula_id) as "Alunos Matriculados"
from turmas t join salas s on t.sala_id = s.sala_id join matriculas m on t.turma_id = m.turma_id
where s.tipo_sala = 'Laboratório'  group by t.nome_turma;
-- -------------------------------------------------------------------------------------------------------------------------------------------------
...
EXPLAIN (ANALYZE, BUFFERS) select p.nome, p.telefone, ROUND(avg(a.nota), 2) as "Media Notas"
from pessoas p join alunos al on p.pessoa_id = al.aluno_id
join matriculas m on al.aluno_id = m.aluno_id
join avaliacoes a on m.matricula_id = a.matricula_id
group by p.pessoa_id, p.nome, p.telefone 
