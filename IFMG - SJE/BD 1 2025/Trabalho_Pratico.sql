...
-- COMANDOS UPDATE
-- 1. Atualiza o e-mail da aluna "Ana Silva" (ID 1), pois ela informou um novo e-mail de contato.
UPDATE vs_alunos
SET email = 'ana.silva.novo@email.com'
WHERE aluno_id = 1;

-- 2. Aumenta o salário da professora "Maria Antunes" (ID 1), pois ela recebeu um aumento salarial por desempenho.
UPDATE vs_professores
SET salario = 5000.00
WHERE professor_id = 1;

-- 3. Altera o status de um pagamento de 'Pendente' para 'Pago', pois a aluna "Olívia Martins" (ID 15) efetuou o pagamento pendente de Outubro de 2024.
UPDATE vs_pagamentos
SET status_pag = 'Pago', data_pag = '2024-10-06' -- Atualiza também a data do pagamento para o dia em que foi efetivado
WHERE aluno_id = 15 AND mes_referencia = '2024-10' AND status_pag = 'Pendente';

-- COMANDOS DELETE
-- O aluno "Thiago Mendes" (ID 20) foi cadastrado, mas decidiu não prosseguir com a matrícula.
DELETE FROM vs_alunos
WHERE aluno_id = 20;

-- A turma 'TURMA_SEM_MATRICULA' (ID 30) foi criada para demonstração e não possui matrículas associadas.
DELETE FROM vs_turmas
WHERE turma_id = 30;
...
-- COMANDOS SELECT
-- A)
-- Busca os professores que possuem salário maior ou igual a 5 mil reais.
select * from vs_professores where (salario >= 5000);

-- Busca as turmas que têm aula na terça ou no sábado.
select * from vs_turmas where dias_semana like 'Ter%' or dias_semana like 'Sab';
...
select p.nome as nome_professor, p.email, p.salario, COALESCE(contagens.total_provas_finais_ingles, 0) AS total_provas_finais_ingles
from vs_professores p right join ( select a.professor_id, COUNT(a.avaliacao_id) as total_provas_finais_ingles from vs_avaliacoes a
join vs_turmas t on a.turma_id = t.turma_id where a.tipo = 'Prova Final' and t.nome like 'ING%' group by
a.professor_id ) as contagens on p.professor_id = contagens.professor_id order by nome_professor;

-- FASE 5
-- A) Adicionando uma nova tabela no esquema
create table vs_material_didatico(
mat_did_id integer not null primary key auto_increment,
nome varchar(50) not null,
descricao varchar(100) not null,
tipo varchar(20),
autor_editora varchar(50) not null,
ano_publicacao integer not null,
curso_id integer,
foreign key (curso_id) references vs_cursos (curso_id)
);

-- B) COMANDOS ALTER TABLE
alter table vs_matriculas add column data_conclusao date;

alter table vs_cursos modify column descricao varchar(200);

alter table vs_avaliacoes alter column nota set default 0.00;

alter table vs_alunos drop column endereco;

-- C) COMANDO DROP TABLE
-- Cria uma tabela temporária para demonstração de exclusão.
create table vs_tabela_temporaria(
tabela_temporaria_id integer not null auto_increment primary key,
nome varchar(50),
data_criacao date default (current_date)
);

-- Regra de Negócio: A tabela vs_tabela_temporaria foi criada para um propósito
-- específico (ex: um teste pontual ou importação de dados temporária) e,
-- após a conclusão da sua finalidade, deve ser removida para manter o banco de dados limpo
-- e otimizado, evitando a acumulação de objetos desnecessários.
-- Exclui a tabela vs_tabela_temporaria.
drop table vs_tabela_temporaria;
